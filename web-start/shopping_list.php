...
<?php 
            ...
            $fileName = "list.json";
            $shoppingList = loadData($fileName);
            if (empty($shoppingList) == true) {
                $shoppingList = [];
            }

        if (empty($_POST) == false) { // if (!empty($_GET)) {
            echo $_POST['title'] . "<br>";
            echo $_POST['store'] . "<br>";
            echo $_POST['quantity'] . "<br>";
            echo $_POST['price']. "<br>";
            // var_dump($_GET); // vyvojarsky vypis :-) 

            $shoppingList[] = $_POST;
            saveData($fileName, $shoppingList);
        }

        // vypsat cely nakupni seznam - foreach
        ?>
        ...
        <div class="col-lg-8 col-lg-offset-2 col-md-10 col-md-offset-1">
            ...
            <?php
                foreach ($shoppingList as $shoppingItem) {
                    ...
                    echo "Polozka: " . $shoppingItem["title"];
                    echo " cena: " . $shoppingItem["price"]; // ' = "
                    echo "<br>";
                }   
            ?>
            </div>
